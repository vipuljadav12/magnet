	...
	class getallimages 
	{
		...
		function getfiles($category,$settings = "",$sortby="",$orderby="") {
	        ...
	        foreach ($getallCat as $key => $value) {
	            $file = explode('.', $value);
	            if (isset($file[1])) {
	                $dir = str_replace('resources/filebrowser', '', $mainDir).'/'.$value;
	                $size = filesize($mainDir.'/'.$value);
                	$file_size = $this->format_size($size);
	                if (in_array($file[1], $extentions)) {
	                    if (in_array($file[1], ['docs','doc'])) {                    
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/docs.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value))];
	                    }elseif ($file[1]=='pdf') {
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/pdf.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value))];
	                    }elseif (in_array($file[1], ['xls','xlsx'])) {
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/xls.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value))];
	                    }else{
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'imgpath'=>$mainDir.'/'.$value,'type'=>'file','filetype'=>'image','access'=>$dir,'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value))];
	                    }
	                }else{
	                    $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/file.png','type'=>'file','filetype'=>'other','access'=>$dir,'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value))];
	                }
            	...
            	foreach ($filefold as $key => $value) {
            		...
            		if($value['type'] == 'folder') {
	                    ...
	                    <figure class="figure"> <a href="javascript:void(0);">
	                        <div class="figure-img"><img src="'.$value["path"].'" class="img-fluid" alt="image" filepath="'.$value["imgpath"].'" filetype="'.$value["filetype"].'" /></div>
		...
		function createNewFolder($folder_name,$directorypath) {
	    	if (!file_exists($directorypath.'/'.$folder_name)) {
			    mkdir($directorypath.'/'.$folder_name, 0777, true);
			    echo "success";
			} else {
				echo "";
			}
	    ...
	    function editFile($data) {
        	...
        	extract($data);
        	$imagePath = $imgpath;
        	$fname = explode('/', $imagePath);
        	$file_name = end($fname);
        	$ext = pathinfo($file_name, PATHINFO_EXTENSION);
        	$fnames = explode('.', $file_name);
        	if($path == "") {
        		$file_path = $_SERVER['DOCUMENT_ROOT'].'/iws/resources/filebrowser/';	
        	} else {
        		$file_path = $_SERVER['DOCUMENT_ROOT'].'/iws/resources/filebrowser/'.$path.'/';
        	}
        	
        	//echo $file_path;exit;
        	$new_image = $file_path.$fnames[0].'_1.'.$ext;
        	//echo $new_image;exit;
            // $ratio = ($t_width/$w1);
            // $nw = ceil($w1 * $ratio);
            // $nh = ceil($h1 * $ratio);
			...
			if ($width > $max_width){
				...
				$uploaded = $this->resizeImage($imagePath,$width,$height,$scale);
			}else{
				$scale = 1;
				$uploaded = $this->resizeImage($imagePath,$width,$height,$scale);
			}
