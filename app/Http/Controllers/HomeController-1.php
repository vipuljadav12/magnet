    ...
    public function index($id="")
    {
       ...
       /* $application_data = Application::where('id',$id)->where('district_id', Session::get('district_id'))->where("status", "Y")->first();
        ...
        if(Session::has("district_id") && Session::get("district_id") != 0)
        {
            ...
            if(!empty($application_data))
            {
                if((strtotime($application_data->starting_date)  <= strtotime(date("Y-m-d H:i:s")) &&  strtotime($application_data->ending_date) >  strtotime(date("Y-m-d H:i:s"))) || (Session::has("from_admin") && strtotime($application_data->admin_starting_date)  <= strtotime(date("Y-m-d H:i:s")) &&  strtotime($application_data->admin_ending_date) >  strtotime(date("Y-m-d H:i:s"))))
                {
                    $district = District::where("id", Session::get("district_id"))->first();
...
//            Session::forget("form_data");
    ...
    public function submit_form($form_id, $form_type)
    {
        ...
        else
        {
            ...
            $emailArr['writing_prompt_due_date'] = getDateTimeFormat($application_data->writing_prompt_due_date);
            $emailArr['recommendation_due_date'] = getDateTimeFormat($application_data->recommendation_due_date);
...
dd($msg, $emailArr);
        ...
        $msg = str_replace("{recommendation_due_date}", $emailArr['recommendation_due_date'], $msg);
        $msg = str_replace("{writing_prompt_due_date}", $emailArr['writing_prompt_due_date'], $msg);
        $parent_upload_link = url('/')."/upload/".$emailArr['application_id']."/grade";
    ...
    public function find_duplicate_submission($form_id, $type="new")
    {
        ...
        if(!empty($condition))
        {
            ...
            if(!empty($dt))
            {
                if(in_array($dt->submission_status, array("Offered and Declined", "Denied due to Ineligibility", "Denied due to Incomplete Records", "Application Withdrawn", "Auto Decline")))
                    return array();
...
//            echo $function;
    ...
    public function recommendationFormPreview($subject, $eligibility_id, $program_id, $application_id)
    {
        ...
        $recommendation_due_date = Application::where('id',$application_id)->first()->recommendation_due_date;
