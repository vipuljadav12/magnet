...
use App\Modules\Waitlist\Export\{WaitlistProcessingExport};
...
class WaitlistController extends Controller
{
    ...
    public function index($application_id = 0, $export = false)
    {
        ...
        if ($export) {
            ...
            return Excel::download(new WaitlistProcessingExport(collect($data_ary)), 'WaitlistProcessing.xlsx');
        } else {
            return view("Waitlist::all_availability_index", compact("application_id", "disp_arr", "display_outcome", "displayother", "last_date_online_acceptance", "last_date_offline_acceptance", "waitlist_process_logs", "swingData", "prgGroupArr"));
        }
    }
    ...
    public function saveSwingData(Request $request, $application_id)
    {
        ...
        return redirect("/admin/Waitlist/Process/Selection/" . $application_id);
    }
    ...
    public function admin_run_selection($application_id = 1)
    {
        ...
        if ($_SERVER['REMOTE_ADDR'] == "49.36.66.6") {
            echo "<pre>";
            print_r($subids);
            exit;
        }
        ...
        foreach ($firstdata as $key => $value) {
            ...
            if ($offered && in_array($offer_type, array('normal', 'OfferedWaitlisted'))) {
                ...
                foreach ($final_data as $fkey => $fvalue) {
                    ...
                    $group_name = $this->program_group[$program_id];
                    if ($fvalue['final_status'] == "Waitlist By Race" && $offer_program_id == $group_name) { //1
    ...
    public function processWaitlist($req, $application_id, $actual_version, $type = "")
    {
        ...
        */
        ...
        foreach ($req['application_program_id'] as $key => $value) {
            if ($req['awardslot' . $value] > 0) {
                ...
                $submissions = Submissions::where('submissions.district_id', Session::get('district_id'))->where(function ($q) {
                    ...
                    ->where('submissions.enrollment_id', Session::get("enrollment_id"))
                    ->where('submissions.next_grade', 'PreK')
                    ->join($table_name, $table_name . ".submission_id", "submissions.id")->where($table_name . ".application_id", $application_id)->where($table_name . ".version", $version)->select("submissions.*", $table_name . ".first_offer_status", $table_name . ".second_offer_status", $table_name . ".first_choice_final_status", $table_name . ".second_choice_final_status")
                    ->get();
        ...
        $race_enroll_arr = [];
        foreach ($arr as $key => $value) {
            $rs = ProgramSwingData::where("application_id", $application_id)->where("program_name", $value)->first();
            $program_swing = $swing;
            if (!empty($rs)) {
                if (is_numeric($rs->swing_percentage) && $rs->swing_percentage > 0) {
                    $program_swing = $rs->swing_percentage;
        ...
        if ($type == "update") {
            ...
            foreach ($interview_arr as $flkey => $flvalue) {
                ...
                $rs = SubmissionsWaitlistFinalStatus::updateOrCreate(["submission_id" => $flvalue['id'], "version" => $actual_version], $insert);
            }
            ...
            /*foreach($failed_arr as $flkey=>$flvalue)
            {
                ...
                if($committee_score_status == "Fail")
                {
                    ...
                    $tmpData['choice_type'] = "first";
                    $tmpData['status'] = "Denied due to Ineligibility";
                    $tmpData['reason'] = "Denied due to Ineligible Records";
                    $rs = SubmissionsTmpFinalStatus::create($tmpData);
            ...
            foreach($incomplete_arr as $flkey=>$flvalue)
            {
                ...
                $tmpData['submission_id'] = $flvalue['id'];
                $tmpData['choice_type'] = "first";
                $tmpData['status'] = "Denied due to Incomplete Records";
                $tmpData['reason'] = "Incomplete Committee Score";
                $rs = SubmissionsTmpFinalStatus::create($tmpData);
        ...
        if ($type == "update") {
            ...
            SubmissionsSelectionReportMaster::where("enrollment_id", Session::get("enrollment_id"))->where("application_id", $application_id)->where("type", "waitlist")->where("version", $actual_version)->delete();
            SubmissionsRaceCompositionReport::where("enrollment_id", Session::get("enrollment_id"))->where("application_id", $application_id)->where("type", "waitlist")->where("version", $actual_version)->delete();
            ...
            $rsUpdate = SubmissionsWaitlistFinalStatus::where("first_choice_final_status", "Offered")->where("application_id", $application_id)->where('version', $actual_version)->where("second_choice_final_status", "Waitlisted")->update(array("second_choice_final_status" => "Pending", "second_waitlist_for" => 0));
            ...
            foreach ($data as $key => $value) {
                ...
                $rs = SubmissionsRaceCompositionReport::updateOrCreate(["application_id" => $application_id, "program_group" => $key, "type" => "waitlist", "version" => $actual_version, "enrollment_id" => Session::get("enrollment_id")], $composition_data);
            }
