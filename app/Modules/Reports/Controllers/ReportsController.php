...
class ReportsController extends Controller
{
    ...
    public function selectionReportVersion($enrollment_id,$application_id, $version=0, $type)
    {
        ...
        if(!empty($process_selecton))
        {
            $data = SubmissionsSelectionReportMaster::where("enrollment_id", Session::get("enrollment_id"))->where("application_id", $application_id)->where("type", $type)->where("version", $version)->get();
            $group_racial_composition = [];
            $rs = SubmissionsRaceCompositionReport::where("enrollment_id", Session::get("enrollment_id"))->where("application_id", $application_id)->where("type", $type)->where("version", $version)->get();
            foreach($rs as $k=>$v)
            {
                $tmp = [];
                $tmp['program_group'] = $v->program_group;
...
//        $enrollment_id = $enrollment_id;
    ...
    /* Missing Grade Report */
    public function missingGradeMain($enrollment_id, $cgrade=8, $late_submission=0)
    {
        $selection = "grade";
        $enrollment = Enrollment::where("district_id", Session::get('district_id'))->get();
    ...
    public function missingGrade($enrollment_id=0, $cgrade=8,  $late_submission=0, $exp_type="")
    {
        set_time_limit(0);
...
$terms = ["2023-24" => ["F1 Grade"], "2022-23" => ["F1 Grade"]];
                            ...
                            if ($empty_ids == true || in_array($program_id, $program_ids)) {
                                ...
                                if(isset($eligibility) && !empty($eligibility)){
                                    ...
                                    $use_calc = array_merge($use_calc, explode(",", $tdata));
                                     ...
                                     if ($value['id'] == 6367) {
                   //                      dd($program_id, $eligibility[0]->assigned_eigibility_name, $use_calc, $score_data, $eligibility[0]->assigned_eigibility_name, $eligibility[0]);
                                     }
...
// echo $enrollment_id;exit;
        ...
        foreach($rs as $k=>$v)
        {
            ...
            $programs = Program::where("report_name", $v->report_name)->where("enrollment_id", $enrollment_id)->select("id", "name")->get()->toArray();
            $tmp_id = $tmp_name = [];
