@php
    ...
    $data = getSubmissionCommitteeScore($submission->id);

@endphp

    @if($eligibility_data->eligibility_type->type=="YN")
        @php $options = $eligibility_data->eligibility_type->YN; @endphp
    @else
        @php $options = $eligibility_data->eligibility_type->NR; @endphp
    @endif 
<form class="form" id="#insterview_score_form" method="post" action="{{url('admin/Submissions/update/CommitteeScore/'.$submission->id)}}">
    {{csrf_field()}}
