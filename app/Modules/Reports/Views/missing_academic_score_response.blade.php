            ...
            <tbody>
                @foreach($data as $key=>$value)
                    <tr id="row{{$value['submission_id']}}">
                        <td class="text-center align-middle"><a href="{{url('/admin/Submissions/edit/'.$value['id'])}}">{{$value['id']}}</a></td>
                        <td class="text-center align-middle">{{$value['student_id']}}</td>
                        <td class="notexport text-center align-middle">{{($value['student_id'] != "" ? "Current" : "Non-Current")}}</td>
                        <td class="text-center align-middle">{{$value['first_name']}}</td>
                        <td class="text-center align-middle">{{$value['last_name']}}</td>
                        <td class="text-center align-middle">{{$value['next_grade']}}</td>
                        <td class="text-center align-middle">{{$value['current_school']}}</td>
                        ...
                        <td class="text-center align-middle">
                            <span @if(!is_numeric($value['academic_score'])) class="scorelabel" @endif>
                                    {!! $value['academic_score'] !!}
                            </span> 
                            @if(!is_numeric($value['academic_score']))
