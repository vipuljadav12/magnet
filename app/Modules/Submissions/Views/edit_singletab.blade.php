  ...
  <div class="card shadow">
    ...
    {{csrf_field()}}                    
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <!-- For choice one -->
            @if($submission->first_choice != '')
                @php $eligibilities = getEligibilitiesDynamic($submission->first_choice); @endphp
            ...
            <!-- For choice two -->
            @if($submission->second_choice != '')
                @php $eligibilities_2 = getEligibilitiesDynamic($submission->second_choice); @endphp
              @foreach($eligibilities_2 as $key=>$value)
                ...
                @if(count($etype_check) <= 0)
                  @if(in_array($value->eligibility_ype, $available_methods_2) && $value->eligibility_ype != "Composite Score")
                      <li class="nav-item"><a class="nav-link" id="choice_2{{$key+1}}-tab" data-toggle="tab" href="#choice_2{{$key+1}}-tabcontent" role="tab" aria-controls="gradeschoice_2{{$key+1}}" aria-selected="false">{{$value->eligibility_ype}}</a></li>
                  @endif
                @endif
              @endforeach
            @endif
...
//                    alert("Email Sent Successfully");
        ...
        function fetchGradeManually()
        {
            $("#wrapperloading").show();
             $.ajax({
                type: "get",
                url: '{{url('/')}}/PowerSchool/fetch_grade_single_manual.php?id={{$submission->id}}',
                success: function(response) {
                    var data = response;
                    /* swal({
