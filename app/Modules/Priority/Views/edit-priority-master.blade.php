...
@section('title')Edit Priority @stop
...
<form id="priority-add" method="post" action="{{url('admin/Priority/update')}}">
    {{  csrf_field() }}
    <!--<div class="card shadow">
        <div class="card-body">
    ...
    <div class="card shadow">
        <div class="card-header">
            <div class="form-group">
                ...
                <div class="">
                    <input type="hidden" id="priority_id" name="id" value="@isset($id){{$id}}@endisset">
                    <input type="text" name="name" value="@if(old('name')){{old('name')}}@elseif(isset($priority->name)){{$priority->name}}@endif" maxlength="30" class="form-control">
                    @if ($errors->has('name'))
                        <span class="error">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped mb-0" id="priority">
                            ...
                            <tbody class="priority-list">
                                @foreach($priority_details as $priority_detail)
                                <tr>
                                    <input type="hidden" name="priority_detail_id[{{$loop->iteration}}]" value="{{$priority_detail->id}}">
                                    <td class=""><a href="javascript:void(0);" class="handle2" title=""><i class="fas fa-arrows-alt"></i></a></td>
                                    <td class=""><input type="text" class="form-control" name="description[{{$loop->iteration}}]" value="@if(old('description.'.$loop->iteration)){{old('description.'.$loop->iteration)}}@elseif(isset($priority_detail->description)){{$priority_detail->description}}@endif">
                                        @if ($errors->has('description.'.$loop->iteration))
                                            <span class="error">
                                                <strong>{{ $errors->first('description.'.$loop->iteration) }}</strong>
                                            </span>
                                        @endif
                                    </td>
                                    
                                    <td class=""><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" name="sibling[{{$loop->iteration}}]" id="{{$loop->iteration+11}}" @if(isset($priority_detail->sibling)&&$priority_detail->sibling=="Y") checked @endif><label for="{{$loop->iteration+11}}" class="custom-control-label"></label></div></td>

                                    @if(isset($dcs) && $dcs!="No")
                                        <td class=""><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" name="majority_race_in_home_zone_school[{{$loop->iteration}}]" id="{{$loop->iteration+22}}" @if(isset($priority_detail->majority_race_in_home_zone_school)&&$priority_detail->majority_race_in_home_zone_school=="Y") checked @endif><label for="{{$loop->iteration+22}}" class="custom-control-label"></label></div>
                                        </td>
                                    @endif

                                    <td class=""><div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" name="current_enrollment_at_another_magnet_school[{{$loop->iteration}}]" id="{{$loop->iteration+33}}" @if(isset($priority_detail->current_enrollment_at_another_magnet_school)&&$priority_detail->current_enrollment_at_another_magnet_school=="Y") checked @endif><label for="{{$loop->iteration+33}}" class="custom-control-label"></label></div>
                                    </td>
                                ...
                                {{-- <tr>
                                    <td class=""><a href="javascript:void(0);" class="handle2" title=""><i class="fas fa-arrows-alt"></i></a></td>
                                    <td class=""><input type="text" class="form-control" name="description[1]" value="@if(old('description[1]')){{old('description[1]')}}@elseif(isset($priority_detail->name)){{$priority->name}}@endif">
                                    </td>
...
<div class="box content-header-floating" id="listFoot">
    <div class="row">
        <div class="col-lg-12 text-right hidden-xs float-right">
            ...
            <a class="btn btn-danger btn-xs" href="{{url('admin/Priority/delete/'.$id)}}"><i class="far fa-trash-alt"></i> Delete</a> 
        </div>
    </div>
</div>
@stop
