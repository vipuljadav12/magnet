 ...
 /* public $statusArr = ["Offered" => "Offer Communication", "Waitlisted" => "Waitlist", "DeniedEligibility" => "Denied due to Ineligibility", "OfferedWaitlisted" => "Offered and Waitlist Option", "DeniedIncomplete" => "Denied due to Incomplete Records"];//,"ContractLetterText" => "Contract Revisit Text"];
  ...
  public function storeLetter(Request $request){
    ...
    public function generate_letter_now($status, $application_id, $preview=false)
    {
        ...
        $rs = ProcessSelection::where("application_id", $application_id)->where("type", "late_submission")->where("version", $version)->where("enrollment_id", Session::get("enrollment_id"))->first();
  ...
  public function storeEmail(Request $request){
    ...
    public function send_email_now($status, $application_id, $preview=false)
    {
        ...
        $rs = ProcessSelection::where("enrollment_id", Session::get("enrollment_id"))->where("application_id", $application_id)->where("enrollment_id", Session::get("enrollment_id"))->where("type", "late_submission")->where("commited", "Yes")->orderBy("created_at", "desc")->first();
        $version = $rs->version;
