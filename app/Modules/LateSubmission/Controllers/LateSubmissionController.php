        ...
        else
        {
            ...
            foreach($rsApplication as $apkey=>$apvalue)
            {
                ...
                if($apvalue->preliminary_processing == "Y")
                {
                    $preliminary = true;
                    $ap_id = $apvalue->id;
                    $count = PreliminaryScore::whereIn("submission_id", function ($query) use ($ap_id){ $query->select('id')->from('submissions')->where("application_id", $ap_id);})->count();
                    if($count == 0)
                    {
                ...
                if(!$preliminary)
                {
                    ...
                    foreach($prgArr as $k=>$v)
                    {
                        ...
                        foreach($rsP as $rk=>$rv)
                        {
                            ...
                            "submission_id", $rv->id)->where("program_id", $v)->first();
                            if(empty($rsT))
                            {
                                $cmt_count++;
                            }
...
//        print_r($availabilityArray);exit;
    ...
    public function loopWaitlistForOffer($final_data)
    {
        ...
        foreach($final_data as $fkey=>$fvalue)
        {
            ...
            {
                if($fvalue['final_status'] == "Waitlist By Race" && $this->offer_program_id == $group_name)
                { //1
