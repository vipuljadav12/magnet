...
//  exit;
    ...
    $rs = $objDB->select("SELECT stateID, student_id FROM student WHERE cdi_fetched='N' LIMIT 5");

    $arr = $arr1 = array();
     for($i=0; $i < count($rs); $i++)
    {
		$SQL = "SELECT stateID FROM student_cdi_details WHERE student_id = '".$value."'";
        ...
        {
            $SQL = "UPDATE student SET cdi_fetched = 'Y' WHERE student_id = '".$rs[$i]['student_id']."'";
   ...
   /* $disciplinary_actions = fetch_inow_details( 'disciplinaryactions' );
    ...
    foreach($arr as $key=>$value)
    {
        ...
        if(count($dispData) > 0)
        {
			...
			$stateID = $arr1[$key];
			$SQL = "SELECT * FROM student_cdi_details WHERE student_id = '".$value."' AND date(datetime) >= '".date("Y-m-d", mktime(0,0,0, 9, 1, (date("Y")-1))) ."' AND (year(datetime) = '".date("Y")."' OR year(datetime) = '".(date("Y")-1)."')";
			//echo $SQL;exit;
			$rs1 = $objDB->select($SQL);
