...
// print_r($_REQUEST);exit;
	...
	if($_POST['action'] == 'searchdirectories') {

		$class = new getallimages;
		$class->searchdirectories($_POST['path'],$_POST['nameToSearch']);
	}
	...
	*/
	class getallimages 
	{
		...
		function getfiles($category,$settings = "",$sortby="",$orderby="") {
			...
			echo '<span id="currentDirectoryPath">'.$mDir.'</span>';
	            ...
	            if (isset($file[1])) {
	                ...
	                if (in_array($file[1], $extentions)) {
	                    if (in_array($file[1], ['docs','doc'])) {                    
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'explct'=>$mainDir.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/docs.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value)),'filesize'=>$size];
	                    }elseif ($file[1]=='pdf') {
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'explct'=>$mainDir.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/pdf.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value)),'filesize'=>$size];
	                    }elseif (in_array($file[1], ['xls','xlsx'])) {
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'explct'=>$mainDir.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/xls.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value)),'filesize'=>$size];
	                    }else{
	                        $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'explct'=>$mainDir.'/'.$value,'imgpath'=>$mainDir.'/'.$value,'type'=>'file','filetype'=>'image','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value)),'filesize'=>$size];
	                    }
	                }else{
	                    $filefold[] = ['name'=>$value,'path'=>$url.'/'.$value,'explct'=>$mainDir.'/'.$value,'imgpath'=>$this->ajaxurl.'/resources/plugins/Imgbrowse/file.png','type'=>'file','filetype'=>'other','access'=>str_replace("/", "", $dir),'size'=>$file_size,'time'=>date('F d y h:i a',filemtime($mainDir.'/'.$value)),'filesize'=>$size];
	                }
	        ...
	        $data .= '<div class="content-wrapper ml-auto">
            	...
            	foreach ($filefold as $key => $value) {
            		...
            		if($value['type'] == 'folder') {
	                    ...
	                    <figure class="figure searchable" draggable="true"> <a href="javascript:void(0);">
	                        <div class="figure-img">';
	                        if($value['filetype'] == "other") {
	                        	$data .= '<img src="'.$value["imgpath"].'?'.rand().'" class="img-fluid" alt="image" filepath="'.$value["path"].'" explct="'.$value["explct"].'" filetype="'.$value["filetype"].'" access="'.$value["access"].'" data-size="1200x800" data-index="'.$key.'" />';
	                        } else {
	                        	$data .= '<img src="'.$value["path"].'?'.rand().'" class="img-fluid" alt="image" filepath="'.$value["imgpath"].'" explct="'.$value["explct"].'" filetype="'.$value["filetype"].'" access="'.$value["access"].'" data-size="1200x800" data-index="'.$key.'" />';
	                        }
	                        $data .= '</div>
		...
		function getdirectories($dir) {
			...
			echo '<span id="currentDirectoryPath">'.$dir.'</span>';
			//$dir = "filebrowser";
			//echo $mainDir;
	        // $getallCat = scandir($mainDir);
	        // unset($getallCat[0]);
	    ...
	    function editFile($data) {
        	...
        	// echo __DIR__;exit();
        	if($path == "") {
        		$file_path = __DIR__.'/../../../filebrowser/'.$file_name;	
        	} else {
        		$file_path = __DIR__.'/../../../filebrowser/'.$path.'/'.$file_name;
        	}
        	// echo $file_path;exit();
   ...
   //          $width = $this->getWidth($imagePath);
	  ...
	  //       imagejpeg($nimg,$imagePath,90);
	    ...
	    function resizeImage() {
	        ...
	        if(strlen($name))
	        {
		        ...
		        if(in_array($ext,$valid_formats))
		        {
			        ...
			        if($path == "") {
		        		$filePath = __DIR__.'/../../../filebrowser/'.$actual_image_name;
		        	} else {
		        		$filePath = __DIR__.'/../../../filebrowser/'.$path.'/'.$actual_image_name;
		        	}
		...
		function createStringPattern($string='')
		{
			$pattern = '';
			...
			for($i=0;$i < strlen($string);$i++)
			{
			 	if(!preg_match("/[^A-Za-z]/", $string[$i]))
	           	{
              		$pattern = $pattern.'['.$string[$i].strtoupper($string[$i]).']';
	           	}
				else
				{
		...
		function searchdirectories($path='',$pattern='')
		{
			$path = isset($path) ? $path : 'fs';
			/*if($path != "") {
				$url = $this->ajaxurl.'/resources/filebrowser/'.$path;
				$mainDir = __DIR__.'/../../../filebrowser/'.$path;
				$mDir = 'resources/filebrowser/'.$path;
			} else {
				$url = $this->ajaxurl.'/resources/filebrowser';
				$mainDir = __DIR__.'/../../../filebrowser';
				$mDir = 'resources/filebrowser';
			...
			// echo $mDir;
				// echo "<br>";
			$dirArr = explode('resources',__DIR__);
			// print_r($dirArr);
				// echo "<br>";
			$newFinalPath =  $dirArr[0].$mDir;
			$newFinalPath = str_replace('/','\\', $newFinalPath);
			// print_r($newFinalPath);
				// echo "<br>";
			// echo (__DIR__);
				// echo "<br>";
			// $pattern = '[tT][eE][sS][tT]';
			$pattern = $this->createStringPattern($pattern);
			$pattern = $newFinalPath.'\*'.$pattern.'*';
			// print_r($files);
	        // $ext = pathinfo($currentPath.'/'.$File, PATHINFO_EXTENSION);
			$extentions = ['jpg','jpeg','gif','png','pdf','docs','doc','xls','xlsx'];
			$imgExts = ['jpg','jpeg','gif','png'];
	        $filefold = [];
			$AllSearchedItems = $this->rglob(str_replace("\\", "/", $pattern));
			 //echo count($AllSearchedItems);
			// print_r($AllSearchedItems);die;
	        $folder = $this->ajaxurl.'/resources/front/assets/images'.'/folder.jpg';
	        $folders = $files = array();
	        $filesHtml = '<ul class="file-list">';
	        $folderHtml = '<div class="row">';
			if(count($AllSearchedItems) > 0)
			{
				foreach ($AllSearchedItems as $key => $single)
			 	{
					if(is_dir($single))
					{
						// echo 'Dir<br>';
						...
						$pathForClick = explode('resources\filebrowser\\',$single);
						$pathForClick = $pathForClick[1];
						// echo "Path---------->".$pathForClick."<br>";
						$folderHtml = $folderHtml.'<span class="img-browser col-sm-auto"><a href="javascript:void(0);" class="folder btn btn-primary d-none d-md-inline-block m-5" getpath="'.$pathForClick.'" > <i class="far fa-folder"></i> <span class="btn-text">'.$name.'</span></a></span><br>';
	        			// print('<img src="'.$folder.'" />');
					}
					else
					{
						// echo 'File<br>';
						$files[] = $single;
						$ext = pathinfo($single, PATHINFO_EXTENSION);
						$urlArr = explode('resources',$url);
						$urlPor = str_replace('/','\\',$urlArr[1]);
						$single = str_replace('/','\\',$single);
						$name = explode('\\', $single);
						$name = $name[count($name)-1];
						$size = $this->format_size(filesize($single));
						$fileDate =date('F d y h:i a',filemtime($single));
						$singleArr = explode($urlPor,$single);
						$newUrl = $url.''.str_replace('\\','/',$singleArr[1]);
						$filesHtml = $filesHtml.'<li class="file-item"><figure class="figure searchable" draggable="true" data-toggle="tooltip" title="<span style=\'color:#ececec;\'>Filebrowser'.$singleArr[1].'</span>" data-placement="right" data-html="true"><a href="javascript:void(0);"><div class="figure-img">';
						// echo $url."<br>".$single."<br>".$urlPor."<br>".$fileDate."<br>".$size."<br>".$newUrl;
						// echo "<br>-----------------<br>";
						// echo $this->ajaxurl.'/resources/plugins/Imgbrowse/xls.png';
						if(in_array($ext, $imgExts))
						{
							// $src = $single;
							$imgSrc = $newUrl;
							$fileType = 'image';
							// /resources/plugins/Imgbrowse/xls.png
						}
						else if($ext == 'xls' || $ext == "xlsx")
						{
							$imgSrc = $this->ajaxurl.'/resources/plugins/Imgbrowse/xls.png';
							$fileType = 'other';
						}
						else if($ext == 'doc' || $ext == "docx")
						{
							$imgSrc = $this->ajaxurl.'/resources/plugins/Imgbrowse/doc.png';
							$fileType = 'other';
						}
						else if($ext == 'pdf')
						{
							$imgSrc = $this->ajaxurl.'/resources/plugins/Imgbrowse/pdf.png';
							$fileType = 'other';
	        				// $imgSrc = $this->ajaxurl.'/resources/front/assets/images'.'/folder.jpg';
						}
						else{
							$imgSrc = $this->ajaxurl.'/resources/plugins/Imgbrowse/file.png';
							$fileType = 'other';
						}
						$filesHtml = $filesHtml.'<img src="'.$imgSrc.'?'.rand().'" class="img-fluid" alt="image" filepath="'.$newUrl.'" explct="'.$single.'" filetype="'.$fileType.'" access="" data-size="1200x800" data-index="0">';					
						$filesHtml = $filesHtml.'</div><figcaption class="figure-caption"><h6>'.$name.'</h6><p><small>'.$fileDate.'</small></p><p><small>'.$size.'</small></p></figcaption></a></figure></li>';
						// print_r($single);
					}//else ends
				}
			}
			$filesHtml = $filesHtml.'<ul></ul></ul>';
			$folderHtml = $folderHtml.'</div>';
			// print_r($folderHtml);
			print_r("<span class='m-5 form-control'>Total ".count($files)." File(s) Found for ".$enterdPattern."</span>");
			print_r($filesHtml);
			// print_r("Nothing To Print".count($files));
			/*print_r(implode('<br>',$folders));
			print_r("<br>-------Files-----<br>");
		...
		/*$base_path =base_path('resources\FS');
		echo $base_path;
		echo "<br>";
		$patt = $base_path.'\*priyank*';
		echo  $patt;
		dd(rglob($patt));*/
